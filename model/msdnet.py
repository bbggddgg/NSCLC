...
"""
Source: https://github.com/kalviny/MSDNet-PyTorch
"""
...
class MSDNet(nn.Module):
    def __init__(self, args, nr_class):
        ...
        for i in range(self.nBlocks):
            ...
            if args.data.startswith('cifar100'):
                ...
                self.classifier.append(
                    self._build_classifier_imagenet(nIn * args.grFactor[-1], nr_class))
            else:
    ...
    def _build_block(self, nIn, args, step, n_layer_all, n_layer_curr):
        ...
        for i in range(step):
            ...
            if args.prune == 'max' and inScales > outScales and \
                ...
                layers.append(
                    self._build_transition(nIn, math.floor(1.0 * args.reduction * nIn), outScales, offset, args))
                _t = nIn
...
def msdnet(pretrained=False, nr_class= 4):
    parser_msdnet = argparse.ArgumentParser()
    parser_msdnet.add_argument('--nBlocks', type=int, default=5)
    parser_msdnet.add_argument('--nChannels', type=int, default=32)
    parser_msdnet.add_argument('--base', type=int, default=4)
    parser_msdnet.add_argument('--stepmode', type=str, default='even', choices=['even', 'lin_grow'])
    parser_msdnet.add_argument('--step', type=int, default=4)
    parser_msdnet.add_argument('--growthRate', type=int, default=16)
    parser_msdnet.add_argument('--grFactor', default=[1, 2, 4, 4], type=list)
    parser_msdnet.add_argument('--prune', default='max', choices=['min', 'max'])
    parser_msdnet.add_argument('--bnFactor', default=[1, 2, 4, 4])
    parser_msdnet.add_argument('--nScales', default=4)
    parser_msdnet.add_argument('--bottleneck', default=True, type=bool)
    parser_msdnet.add_argument('--data', default='ImageNet')
    parser_msdnet.add_argument('--reduction', default=0.5, type=float,
                            metavar='C', help='compression ratio of DenseNet'
                                              ' (1 means dot\'t use compression) (default: 0.5)')
    args_msdnet = parser_msdnet.parse_args()
    model = MSDNet(args_msdnet, nr_class=nr_class)
    if pretrained:
        # model = torch.nn.DataParallel(model).to('cuda')
        default_ckpt = os.path.join(torch.hub.get_dir(), "checkpoints", "msdnet-step=4-block=5.pth.tar")
        pre_trained_path = os.environ.get("MSDNET_PRETRAINED", default_ckpt)
        saved_state = torch.load(pre_trained_path)['state_dict']
        new_state_dict = OrderedDict()
        for key, value in saved_state.items():
            new_key = key[7:]
            if 'linear' in new_key:
                new_key = new_key.replace('linear', 'linear_')

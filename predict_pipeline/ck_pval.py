...
if __name__ == '__main__':
    ...
    stain_path = os.path.join(example_root, "test_case", "tissue", "Lite-MMT-Net", "ck_2.csv")
    ...
    def generate_label_pred(row):
        ...
        if row['viable_tumor'] == max_score or row['necrosis or keratinization'] == max_score:
            return 1
        else:
            return 0
    ...
    stain_df['label_stain'] = stain_df['ratio'].apply(lambda x: 1 if x > 0.1 else 0)
    ...
    # 打印结果
    print(region_stats)  # 查看前几个区域的统计结果
    ...
    # 添加标题和标签
    plt.title("与CK免疫组化相关性", fontsize=24)
    plt.xlabel("预测的肿瘤区域比例", fontsize=20)
    plt.ylabel("CK染色阳性区域比例", fontsize=20)
    ...
    # 动态设置注释位置
    text_x = x_min + (x_max - x_min) * 0.65  # x轴起点右侧 5%
    text_y = y_max - (y_max - y_min) * 0.95  # y轴顶部下方 10%
    ...
    plt.tight_layout()
    save_path = os.path.join(example_root, "test_case", "tissue", "Lite-MMT-Net", "correlation_scatter_plot_ck.png")

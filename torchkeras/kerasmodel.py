...
def warmup_lr_scheduler(optimizer, warmup_iters, warmup_factor):
    def f(x):
        if x >= warmup_iters:
            return 1
        alpha = float(x) / warmup_iters
        new_lr = warmup_factor * (1 - alpha) + alpha
        # print(f"Step {x}, New LR: {new_lr}")
        return new_lr
...
class StepRunner:
    def __init__(self, net, loss_fn, accelerator, stage = "train", metrics_dict = None, 
                 optimizer = None, warmup_lr_sche = None):
        self.net,self.loss_fn,self.metrics_dict,self.stage = net,loss_fn,metrics_dict,stage
        self.optimizer = optimizer
        self.accelerator = accelerator
        self.warmup_lr_scheduler = warmup_lr_sche
...
class EpochRunner:
    def __init__(self,steprunner, lr_scheduler=None, quiet=False, warmup=False):
        self.steprunner = steprunner
        self.stage = steprunner.stage
        self.accelerator = self.steprunner.accelerator
        self.lr_scheduler = lr_scheduler  # 添加 lr_scheduler 参数并保存为实例变量
        self.quiet = quiet
    ...
    def __call__(self,dataloader):
        ...
        if self.lr_scheduler is not None:
            self.lr_scheduler.step()  # 在每个epoch结束后调用 lr_scheduler.step()
...
class KerasModel(torch.nn.Module):
    ...
    def fit(self, train_data, val_data=None, epochs=10,ckpt_path='checkpoint.pt',
            ...
            plot=False, wandb=False, quiet=False, save_best_only=True, warmup=True):
        ...
        for epoch in range(start_epoch,epochs+1):
            ...
            if warmup==True and epoch==1:
                warmup_factor = 1. / 1000
                warmup_iters = min(1000, len(train_data) - 1)
                self.warmup_lr_sche = warmup_lr_scheduler(self.optimizer,warmup_iters, warmup_factor)
                self.warmup_lr_sche = self.accelerator.prepare(self.warmup_lr_sche)
            else:
                self.warmup_lr_sche = None
            # 1，train -------------------------------------------------  
            train_step_runner = self.StepRunner(
                    ...
                    warmup_lr_sche=self.warmup_lr_sche
            )

            train_epoch_runner = self.EpochRunner(train_step_runner,self.lr_scheduler, should_quiet)
            ...
            if val_dataloader:
                ...
                val_epoch_runner = self.EpochRunner(val_step_runner, None, should_quiet)
                with torch.no_grad():
            ...
            if best_score_idx==len(arr_scores)-1:
                ...
                best_score = arr_scores[best_score_idx]
                if save_best_only == True:
                    self.accelerator.save(net_dict,ckpt_path)
                else:
                    mod = 'epoch{}_{}_{}.pt'.format(str(epoch), str(monitor), str(best_score))
                    best_ckpt_path = ckpt_path.replace('.pt', mod)
                    self.accelerator.save(net_dict, best_ckpt_path)
                    self.accelerator.save(net_dict, ckpt_path)
                if not should_quiet:
